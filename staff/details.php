...
<?php
	...
	{ 
        ...
        $update = '';
        if ($urgency) {
            $sql = "update tblrequest set registra_remarks=:description,Status=:status,admin_status=:adminStatus,AdminRemarkDate=:admremarkdate where id=:did";
            $query = $dbh->prepare($sql);
            ...
            $sql = "update tblrequest set registra_remarks=:description,Status=:status,AdminRemarkDate=:admremarkdate where id=:did";
            $query = $dbh->prepare($sql);
            $query->bindParam(':description', $description, PDO::PARAM_STR);
            $query->bindParam(':status', $status, PDO::PARAM_STR);
            $query->bindParam(':admremarkdate', $admremarkdate, PDO::PARAM_STR);
        ...
        if ($urgency) {
            ...
            $update = "update tblrequest, tblemployees set tblrequest.registra_remarks='$description',tblrequest.Status='$status',tblrequest.AdminRemarkDate='$admremarkdate' where tblrequest.empid = tblemployees.emp_id AND tblrequest.id='$did'";
                    ...
                    <form method="post" action="">
                        ...
                        <div class="row">
                           ...
                           <div class="col-md-3">
                            ...
                            if ($adminStatus == 0) {
                                ...
                                <div class="col-md-2">
                                    <div style="gap:0.5rem;" class="d-flex justify-content-center align-items-baseline">
                                        ...
                                        <label for='urgent' class='font-weight-bold'>Admin Urgency</label>
	                        ...
	                        <input type="hidden" id="getStatus" value='<?php echo $staffStatus ?>'>

                            <?php 
                            if(($stats==0 AND $ad_stats==1) OR ($stats==2 AND $ad_stats==0) OR ($stats==2 AND $ad_stats==2) || $adminStatus == 0)
                            // echo "Stats: $stats, Ad Stats: $ad_stats"; // Debugging line
