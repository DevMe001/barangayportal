...
<?php
	$isread=1;
	$did=intval($_GET['requestid']);  
	date_default_timezone_set('Asia/Kolkata');
	$admremarkdate=date('Y-m-d G:i:s ', strtotime("now"));
	$sql="update tblrequest set IsRead=:isread where id=:did";
	...
	{ 
		...
		if ($urgency) {
			$sql = "update tblrequest set registra_remarks=:description,Status=:staffstatus,admin_status=:status,AdminRemarkDate=:admremarkdate where id=:did";

			$query = $dbh->prepare($sql);
			$query->bindParam(':description', $description, PDO::PARAM_STR);
			$query->bindParam(':staffstatus', $status, PDO::PARAM_STR);
			...
			$sql = "update tblrequest set registra_remarks=:description,admin_status=:status,AdminRemarkDate=:admremarkdate where id=:did";
		...
		elseif ($status === '1') {
				$update = '';
				if($urgency){
				$update	="update tblrequest, tblemployees set tblrequest.registra_remarks='$description',tblrequest.Status='$status',tblrequest.admin_status='$status',tblrequest.AdminRemarkDate='$admremarkdate' where tblrequest.empid = tblemployees.emp_id AND tblrequest.id='$did'";
				}
				else{
				$update = "update tblrequest, tblemployees set tblrequest.registra_remarks='$description',tblrequest.admin_status='$status',tblrequest.AdminRemarkDate='$admremarkdate' where tblrequest.empid = tblemployees.emp_id AND tblrequest.id='$did'";
				...
				<div class="pd-20 card-box mb-30">
					...
					<form method="post" action="">
						...
						<div class="row">
							...
							<div class="col-md-4">
								<div class="form-group">
									<label style="font-size:16px;"><b>Request Period</b></label>
						...
						<div class="form-group row">
								<label style="font-size:16px;" class="col-sm-12 col-md-2 col-form-label"><b>Request Reason</b></label>
						...
						<div class="row">
							...
							<div class="col-md-3">
								<div class="form-group">
									<label style="font-size:16px;"><b>Request Status From HOD</b></label>
						...
						<?php $staffStatus = $result->Status;
										...
										if ($staffStatus == 0) {
											?>
												...
												<div class="col-md-2">
											<div style="gap:0.5rem;" class="d-flex justify-content-center align-items-baseline">
										
													<input  type="checkbox" name="urgency" id="urgent">

													<label for='urgent' class='font-weight-bold'>Staff Urgency</label>
							...
							<div class="col-md-2">
								<div class="form-group">
									...
									<input type="hidden" id="getStatus" value='<?php echo $stats ?>'>

							<?php 
							if(($stats==0 AND $ad_stats==0) OR ($stats==2 AND $ad_stats==0) OR ($stats==2 AND $ad_stats==2) || $staffStatus == 0)
							  {

							 ?>
							<div id='urgentAction' class="col-md-3">
								<div class="form-group">
									...
									<div class="modal-dialog modal-dialog-centered" role="document">
										<div class="modal-content">
											<div class="modal-body text-center font-18">
												<h4 class="mb-20">Request take action</h4>
...
let checking = $('#urgent').is(':checked');

let admin_status = $('#getStatus').val();

if(!checking && admin_status != 0){
	$('#urgentAction').addClass('d-none');
}
	...
	$('#urgent').on('click',function(){
		...
		if(isCheck){
			$('#urgentAction').removeClass('d-none')
		}else{
			$('#urgentAction').addClass('d-none');
